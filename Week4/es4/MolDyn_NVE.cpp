...
void Input(void){ //Prepare all stuff for the simulation
  ...
  for(int i=0;i<nblocks;i++) { ave[i] = new double[4+100];}
...
//Read initial configuration
  ...
  ReadConf.close();
   ...
   if(restart){
	...
	for(int i=0; i<npart; i++){			// check center of mass velocity
	...
	for(int i=0; i<npart;i++){ 			//fix Center of Mass velocity to zero
	...
	/*double check[3]={0.,0.,0.};			//check average is zero
	...
	*/
	...
	for(int i=0; i<npart; i++){   //rescale velocities to obtain desired temp
   ...
   Epot.open("output_epot.dat",ios::trunc); // clear output files
   ...
   Epot.open("ave_epot.out",ios::trunc); // clear output files
...
void Average(int iblock){
  ...
  ave[iblock-1][iv] = Epot_ave;   // ip = 0 = Potential
  ave[iblock-1][ik] = Ekin_ave;   // ik = 1 = Kinetic
  ave[iblock-1][ie] = Etot_ave;   // ie = 2 = Total Energy
  ave[iblock-1][it] = Temp_ave;   // it = 3 = Temperature
  
  for(int ibin = 0;ibin <nbins;ibin++){
  	ave[iblock-1][ibin] = g_ave[ibin];
	g_ave[ibin] = 0;
  }
...
void Print_Ave(){
	...
	double prog_sum[4+100];
	double prog_sq[4+100];
	double error[4+100];
	ofstream Epot, Ekin, Etot, Temp, G_ave;


	for(int q=0;q<4+100;q++) {    // q for quantity
	...
	for(int ibin = 0; ibin <nbins;ibin++){
		G_ave << prog_sum[4+ibin] << " " << error[4+ibin] << " ";
		G_ave << endl;
	}
